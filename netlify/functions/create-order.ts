...
export default async (request: Request) => {
  ...
  try {
    ...
    let siteUrl = "https://saatvikannafoods.in";
    if (originHeader) {
      try {
        const parsedOrigin = new URL(originHeader);
        siteUrl = `${parsedOrigin.protocol}//${parsedOrigin.host}`;
      } catch {}
    }

    const cashfreePayload = JSON.stringify({
      order_id: orderId,
    ...
    const cashfreeHeaders: Record<string, string> = {
      "Content-Type": "application/json",
    ...
    let cashfreeResponse: { ok: boolean; status: number; text: () => Promise<string> };

    try {
      cashfreeResponse = await fetch("https://api.cashfree.com/pg/orders", {
        method: "POST",
        headers: cashfreeHeaders,
        body: cashfreePayload,
      });
    } catch (fetchErr) {
      ...
      cashfreeResponse = await new Promise((resolve, reject) => {
        const req = https.request(
          {
            ...
            headers: {
              ...cashfreeHeaders,
              Host: url.hostname,
              "Content-Length": String(Buffer.byteLength(cashfreePayload)),
            },
          },
          (res) => {
            let data = "";
    ...
    console.error("Cashfree create-order catch error:", error);
    return new Response(
      JSON.stringify({
